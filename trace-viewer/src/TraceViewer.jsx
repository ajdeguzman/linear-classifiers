    ...
    const fetchData = async () => {
      try {
        ...
        if (url.endsWith('.gz')) {
          ...
          const decompressed = inflate(new Uint8Array(response.data), { to: 'string' });
...
function renderMarkdown(content) {
  ...
  // Extract display math $$...$$ before marked sees it
  let processed = content.replace(/\$\$([\s\S]*?)\$\$/g, (_, math) => {
    const id = mathSlots.length;
    mathSlots.push({ display: true, math: math.trim() });
    return `XKATEX${id}X`;
  });

  // Extract inline math $...$ (single dollar, no newlines inside)
  processed = processed.replace(/\$([^$\n]+?)\$/g, (_, math) => {
    const id = mathSlots.length;
    mathSlots.push({ display: false, math: math.trim() });
    return `XKATEX${id}X`;
  });

  // Run marked on math-free text
  let html = marked(processed);

  // Restore math as KaTeX HTML (alphanumeric placeholder survives marked unchanged)
  html = html.replace(/XKATEX(\d+)X/g, (_, id) => {
    const { display, math } = mathSlots[parseInt(id)];
    try {
  ...
  return html + trailingWhitespace;
}

function MarkdownRenderer({ content, style }) {
  return <span className="markdown" style={style} dangerouslySetInnerHTML={{ __html: renderMarkdown(content) }} />;
}
